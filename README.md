...
> synthetic. When it is connected to Microsoft Foundry, three collaborating
...
If you are new to agentic applications, Microsoft Foundry, DevOps, MLOps,
...
1. [Getting started for new teams](docs/getting-started-for-new-teams.md)
   — concept-first tour of what an agent, a coordinator, and Microsoft
...
The three agents run on **Microsoft Foundry** through Microsoft Agent
...
- The language-model calls happen on Microsoft Foundry once the setup
...
- [`infra`](infra/README.md) — Terraform for Microsoft Foundry.
- [`workshop`](workshop/README.md) — the hands-on workshop, modules 0-10.
- [`demo`](demo/README.md) — the customer demo walkthrough.
...
Install these once on the machine that will run the app.
...
| Terraform | 1.9+ | Provisions Microsoft Foundry infrastructure. | `terraform version` |
...
- **Model quota** in the region you choose. Quota is per-region and
  ...
  on `DataZoneStandard` in your chosen region before you start.
- The **subscription ID** that will host it. `az account show
...
- Cost is **pay-as-you-go** based on the model deployment and telemetry.
  For intermittent use, expect single-digit USD per day. Run
...
The application makes **real model calls to Microsoft Foundry**. There
is no runtime mock mode. If Foundry is not configured, the
recommendation endpoint fails with a clear setup error and the in-app
Setup Status panel says so.
...
**Step 3 — deploy Microsoft Foundry infrastructure.**
...
> recommendation request returns 401 or 403, wait a moment and retry.
...
Look for `"active_provider": "azure_foundry_responses"` and
`"customer_demo_ready": true`. If either is wrong, the same payload names
the missing piece.
...
- `terraform apply` fails on the deployment. Options:
  ...
  - If `DataZoneStandard` is not available in your region, fall back to
    `GlobalStandard` and note the change in your own notes.
...
- `scripts/publish_prompt_agents.py` — publish the prompt agents to Foundry.
  ...
  `--delete` removes the agents published under exactly that suffix.
...
- [Customer demo](demo/README.md) — the 5–7 minute walkthrough.
